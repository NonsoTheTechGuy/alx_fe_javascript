...
// Add lastModified to each quote for conflict resolution
function addTimestampToQuote(quote) {
  return { ...quote, lastModified: Date.now() };
}

// Update add-quote logic to include lastModified
function createAddQuoteForm() {
  if (document.getElementById('addQuoteForm')) return;
  ...
  form.addEventListener('submit', function(e) {
    ...
    const msg = document.getElementById('addQuoteMsg');
    if (text && category) {
      quotes.push(addTimestampToQuote({ text, category }));
      saveQuotesToLocal();
      showRandomQuote();
      msg.textContent = "Quote added!";
      setTimeout(() => form.remove(), 1000);
...
// Merge server and local quotes, resolving conflicts by lastModified
function mergeQuotes(localArr, serverArr) {
  const merged = [];
  const map = new Map();

  // Index by text+category (simple key, can be improved)
  [...localArr, ...serverArr].forEach(q => {
    const key = q.text + '|' + q.category;
    if (!map.has(key) || (q.lastModified > map.get(key).lastModified)) {
      map.set(key, q);
    }
  });

  map.forEach(q => merged.push(q));
  return merged;
}
...
// Push local quotes to server (overwrites server data)
...
syncFromBtn.textContent = 'Sync From Server';
syncFromBtn.onclick = syncFromServer;
document.body.appendChild(syncFromBtn);

const syncToBtn = document.createElement('button');
syncToBtn.textContent = 'Sync To Server';
syncToBtn.onclick = syncToServer;
document.body.appendChild(syncToBtn);